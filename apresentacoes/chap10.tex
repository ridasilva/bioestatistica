...
\begin{itemize}
\item
  Temos que como \(\gamma/2 = 0,475\), devemos procurar por \(z_{\gamma/2}\) cuja área do corpo
  da tabela de 0,975;
\item
...
\begin{itemize}
\item a estimativas pontuais $\bar{x}=155.67$, $S=20.85$ e $n=60$;
...
O valor t crítico vem da tabela t crítica bicaudal 0.05 porque estamos computando um IC de 95\% de confiança. O \textit{gl} é 59 (gl = n - 1 = 60 - 1 = 59), então o \(t_{IC}\) é 2.001. E os \(SEM_a\) é \(DP/\sqrt{N}\) ou \(20.85/\sqrt{60}\) = 2.69.
...
Deseja-se investigar se uma certa moléstia que ataca o rim altera o consumo de oxigênio desse orgão. De uma amostra (\(n=5\)) Foram obtidos \(\bar{x}=13.9\) e \(S^2=0.67\). Determine o intervalo de confiança para \(\mu\) com \(\gamma=0.95\).
\vspace{1in}
\vspace{1in}
...
Em um estudo sobre a deficiência de zinco em crianças, foi utilizada uma amostra de \(n=35\) crianças. Em amostras de sangue dessas crianças, obtidas por punção venosa, foi encontrada uma média de \(\bar{x}=118.7\ \mu g/dl\) para os níveis séricos de zinco, com desvio padrão amostral de \(S=23.1 \mu g/dl\). Determine o intervalo de confiança para \(\mu\) com \(\gamma=0.99\).
\vspace{1in}
\vspace{1in}
