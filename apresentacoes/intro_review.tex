...
\\~\\
  ...
  \uncover<4->{\item
    Dúvidas podem ser enviadas por email: \href{mailto:ridasilva@usp.br}{ridasilva@usp.br}, Monitor - \href{mailto:joao.vitor.ferreira@usp.br}{joao.vitor.ferreira@usp.br}}
  \end{itemize}
...
\\~\\
  \begin{itemize}
  \uncover<1->{\item
    \(N = \frac{2 \times E+4 \times P1+4 \times P2}{10}\), onde \(E=\frac{1}{n}\sum_{i=1}^nN_i\) representa \textbf{atividades feitas em sala de aula}, e \(P\) representa as provas 1 e 2;}
  \uncover<2->{\item
    Da segunda aula em diante teremos um \textbf{teste ao final de cada aula};}
  \uncover<3->{\item
    O teste contará como chamada. Apenas quem somar 5 ou mais não necessitará de frequência mínima para aprovação;}
  \uncover<4->{\item
    O uso de computadores em sala de aula será estimulado;}
   \uncover<5->{\item
    O Class Room será utilizado para compartilhamento de material e discussão de dúvidas;}
    \uncover<6->{\item
    É possível optar por ter os pontos dos questionários semanais adicionados às provas. \url{https://forms.gle/S2rhtNzzcH17XX1B9}}    
  \end{itemize}
